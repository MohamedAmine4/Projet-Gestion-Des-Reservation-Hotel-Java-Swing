...
public class Ajouter_RES extends JPanel  {
     int numch1=0,numch2=0,numch3=0;
	...
	String value_prichambre1,value_prichambre2,value_prichambre3;
...
//		btnadulte.add(adulte_2);
	...
	public String getValue_prichambre1() {
		return value_prichambre1;
	}


	public String getValue_prichambre2() {
		return value_prichambre2;
	}


	public String getValue_prichambre3() {
		return value_prichambre3;
	}
...
//		dated
  ...
  if(date_arr==null || date_de==null) {
   ...
   //static LinkedList<Integer> listnumch1=new LinkedList<>();
   //static LinkedList<Integer> listnumch2=new LinkedList<>();
   //static LinkedList<Integer> listnumch3=new LinkedList<>();
   
   static int numeroch1, numeroch2, numeroch3;
   
   private void Enregistrer(ActionEvent e) {
	  // int r=connecter.getAllDataDerIdRes("Reservation");
	    //r++;
	   Object s=e.getSource();
	   
	   int idSaison=0;
	   if(saison().compareTo("basse-saison")==0) idSaison=1;
	     else if(saison().compareTo("moyenne-saison")==0) idSaison=2;
	     else   idSaison=3;
...
/////////////////////////enregistrer chambre 1////////////////////////////////////
	   if(s==btnenregistrer1) {	 
		   ...
		   int test=0;
		   try {
			    String input = adulte_1.getText().trim();
			    if (input.isEmpty()) {
			        JOptionPane.showMessageDialog(null, "Saisir un nombre d'adulte ");
			        adulte_1.setText("");
			        test=1;
			        return ;
			    }
			    int nbrAdulte = Integer.parseInt(input);
			    if (nbrAdulte < 1 || nbrAdulte > 2) {
		   ...
		   if(test==1) {  }
			   ...
			   JOptionPane.showMessageDialog(null,"La Chambre 1 a été Enregistré avec succés ");
			     ...
			     if(chambre_s.isSelected()) {
			    	int  nbrAdulte = Integer.parseInt(adulte_1.getText().trim());
			    
			    	 int categorie=1;
			    	int numch= connecter.Numch(categorie);
			    	
			    	if(numch==0) JOptionPane.showMessageDialog(null, "Nous sommes désolés, mais la chambre de la catégorie "+ categorie+" que vous avez sélectionnée n'est malheureusement pas disponible en ce moment.\n Y aurait-il une autre catégorie qui pourrait vous intéresser ? \nJe serais heureux de vous aider à trouver une chambre qui convient parfaitement à vos besoins.");
			    	 
			    	 else {
			    		 numeroch1=numch;
			    		 enre1++;
			    		// listnumch1.add(numch);
			    		 JOptionPane.showMessageDialog(null, "Votre numéro de Chambre est  "+numch+" avec Categorie "+categorie);
...
//					  System.out.println("enfant"+ nbrEnfant);
					 ...
					 // System.out.println(value_chambre1);
					  ...
					  if( nbrAdulte==2 && nbrEnfant==2) { prix_s=(prix_s*2)+250; }
						else if ( nbrAdulte==2 && (nbrEnfant==0 || nbrEnfant==1)) { prix_s=(prix_s*2);}
						     ...
						     if(saison().compareTo("basse-saison")==0) prix_s+=prix_s*0.05;//augmenter de  5%
						       ...
						       //System.out.println("categorie 1");
							     ...
							     //  String tablePCh="PrixChambre" ;

							        value_prichambre1="'"+categorie+"', "+ idSaison+", "+numeroch1+", '"+prixch1+"'";
							     //   connecter.insertData(tablePCh, value_prichambre1);
			    }}
				  ...
				  int categorie=2;
			    	...
			    	if(numch==0) JOptionPane.showMessageDialog(null, "Nous sommes désolés, mais la chambre de la catégorie "+ categorie+" que vous avez sélectionnée n'est malheureusement pas disponible en ce moment. Y aurait-il une autre catégorie qui pourrait vous intéresser ? Je serais heureux de vous aider à trouver une chambre qui convient parfaitement à vos besoins.");
...
//					  System.out.println("enfant"+ nbrEnfant);
					  ...
					  if( nbrAdulte==2 && nbrEnfant==2) { prix_d=(prix_d*2)+500; }
					   else if ( nbrAdulte==2 && (nbrEnfant==0 || nbrEnfant==1)) { prix_d=(prix_d*2);}
					        else if (nbrAdulte==1 && nbrEnfant==2) { prix_d+=500;}
				                  else  { prix_d=prix_d;}
			    	 ...
			    	 if( nbrAdulte==2 && nbrEnfant==2) { prix_su=(prix_su*2)+600; }
					   else if ( nbrAdulte==2 && (nbrEnfant==0 || nbrEnfant==1 )) { prix_su=(prix_su*2);}
					         else if (nbrAdulte==1 && nbrEnfant==2) { prix_su+=600;}
...
/////////////////////////enregistrer chambre 2////////////////////////////////////
		   ...
		   int test=0;
		   try {
			    String input = adulte_2.getText().trim();
			    if (input.isEmpty()) {
			        JOptionPane.showMessageDialog(null, "Saisir un nombre d'adulte ");
			        adulte_2.setText("");
			        test=1;
	  ...
	  // enre2++;
	   ...
	   if(chambre_s2.isSelected()) {
		   int  nbrAdulte = Integer.parseInt(adulte_2.getText().trim());
		   int categorie=1;
	    	int numch= connecter.Numch(categorie);
			  ...
			  if( nbrAdulte==2 && nbrEnfant==2) { prix_s=(prix_s*2)+250; }
			     ...
			     if(saison().compareTo("basse-saison")==0) prix_s+=prix_s*0.05;//augmenter de  5%
			        ...
			        prix_s=1000;
						      ...
						      // String tablePCh="PrixChambre" ;
						       value_prichambre2="'"+categorie+"', "+ idSaison+", "+numeroch2+", '"+prixch2+"'";
						     //   connecter.insertData(tablePCh, value_prichambre2);
						       }}
				    	...
				    	//	listnumch2.add(numch); 
				    		...
				    		// connecter.updateetat(numch);
						     ...
						     if(saison().compareTo("basse-saison")==0) prix_su+=prix_su*0.05;//augmenter de  5%
						       ...
						       value_chambre2="'"+3+"', "+ numch+", "+nbrAdulte+", '"+nbrEnfant+"'"; 
	   ...
	   /////////////////////////enregistrer chambre 3////////////////////////////////////
		    ...
		    // enre3++;
		     int nbrEnfant=Integer.parseInt(btwnpm3.getText()); 
			   int test=0;
			   try {
				    String input = adulte_3.getText().trim();
				    if (input.isEmpty()) {
				        JOptionPane.showMessageDialog(null, "Saisir un nombre d'adulte ");
				        adulte_3.setText("");
				        test=1;
		  ...
		  if(chambre_s3.isSelected()) {
			  int  nbrAdulte = Integer.parseInt(adulte_3.getText().trim());
			  int categorie=1;
		    	int numch= connecter.Numch(categorie);
		    	...
		    	if(numch==0) JOptionPane.showMessageDialog(null, "Nous sommes désolés, mais la chambre de la catégorie "+ categorie+" que vous avez sélectionnée n'est malheureusement pas disponible en ce moment. Y aurait-il une autre catégorie qui pourrait vous intéresser ? Je serais heureux de vous aider à trouver une chambre qui convient parfaitement à vos besoins.");
			     ...
			     //  String tablePCh="PrixChambre" ;
			        value_prichambre3="'"+categorie+"', "+ idSaison+", "+numeroch3+", '"+prixch3+"'";
			      //  connecter.insertData(tablePCh, value_prichambre3);
		  }
		  }
  ...
  // public static LinkedList<Integer> listNumChRe(){
	//   return listnumch;
   //}
   
   private void Annuler(ActionEvent e) {
	   Object s=e.getSource();
   	int h=JOptionPane.showConfirmDialog(null,"Souhaitez-vous vraiment annuler la reservation ?","",JOptionPane.YES_NO_OPTION);
